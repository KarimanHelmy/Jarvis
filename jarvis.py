...
apiCorona = 't1-Vxp2Jn1AY'
coronaToken = 't_iUNRwcg4Ta'
runToken = 'tShhxup6JBY8'

updateCoronaCases = "update"
...
class Data:
    ...
    def __init__(self, api_key, project_token):
        ...
        self.params = {
            "api_key": self.api_key  # so we don't have to authenticate every time
        }
        self.data = self.data_get()

    def data_get(self):
        responses = requests.get(f'https://www.parsehub.com/api/v2/projects/t_iUNRwcg4Ta/last_ready_run/data',
                                 params={'api_key': apiCorona})  # get latest run I did
        data = json.loads(responses.text)
        return data

    def total_cases(self):
        data = self.data['selection1']
        for content in data:  # to get content name you have to run it first once to see what is the name you are searching for
            if content['name'] == "Coronavirus Cases:":  # or just go to csv file
    ...
    def get_total_deaths(self):
        ...
        for content in data:
            if content['name'] == "Deaths:":
                return content['value']  # when using pasre I decided to name the link value so I already know

    def new_cases(self):
        data = self.data['countries']
        for content in data:
            if content['name'] == "newCases:":
                return content['value']

    def active_cases(self):
        data = self.data['countries']
        for content in data:
            if content['name'] == "activecases:":
                return content['value']
    ...
    def update_data(self):
        responses = requests.get(f'https://www.parsehub.com/api/v2/projects/t_iUNRwcg4Ta/run',
                                 params=self.params)
...
def get_events(day, service):
    ...
    if not events:
        ...
        for event in events:
            ...
            if int(start_time.split(":")[
                       0]) < 12:  # if the event is in the morning we don't need to convert it to 12 hours format
                start_time = start_time + "am"
            else:
                # int utc
                start_time = str(int(start_time.split(":")[0]) - 12) + start_time.split(":")[
                    1]  # convert 24 hour time to regular
                start_time = start_time + "pm"
            speak(event["summary"] + " at " + start_time)


data = Data(apiCorona, coronaToken)
country_list = data.getListCountries()
...
countryCases = {
    re.compile("[\w\s]+ cases [\w\s]+"): lambda country: data.get_country_data(country)['totalCasesCountry'],
    re.compile("[\w\s]+ deaths [\w\s]+"): lambda country: data.get_country_data(country)['TotalDeath'],
    re.compile("[\w\s]+ death [\w\s]+"): lambda country: data.get_country_data(country)['TotalDeath'],
    re.compile("[\w\s]+ new cases [\w\s]+"): lambda country: data.get_country_data(country)['newCases'],
...
print(
    "for mathmatical operation or any equations it may be better to type as I am still learning so I may get it wrong")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for pattern, func in countryCases.items():
        ...
        for pattern, func in totalWorldCases.items():
            if pattern.match(text):
                result = func()
                print(result)
                speak(result)
        ...
        for word in forecast:
            if word in text:
                ...
                if reg_ex:
                    city = reg_ex.group(1)
                    location = owm.weather_at_place(
                        city)  # if you get error in this part reinstall with this version pip install pyowm==2.10.0
